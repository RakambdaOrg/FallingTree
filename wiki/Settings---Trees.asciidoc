...
- ABOVE_CUT: ABOVE_CUT will only follow a path of logs that goes up, never down.
...
|Min (break) size
|The minimum size of a tree. If a tree with less elements than this value is attempted to be cut, the mod will skip it.
...
NOTE: Does not apply to `SHIFT_DOWN` breaking mode.
...
|Spawn items at break point
|If enabled, all logs will spawn where the log was broken instead of at their own location.

INFO: Only in INSTANTANEOUS mode.

|Trunk loot percentage
|Percentage of the logs that make up the trunk to be dropped as items.

INFO: Does not apply to SHIFT_DOWN mode.
|===
