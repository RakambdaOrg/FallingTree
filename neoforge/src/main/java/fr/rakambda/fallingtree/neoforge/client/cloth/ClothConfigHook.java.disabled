package fr.rakambda.fallingtree.neoforge.client.cloth;
...
public class ClothConfigHook extends ClothHookBase{
	...
	public void load(ModContainer modContainer){
	...
	@NonNull
	private Component translatable(@NonNull String key){
		return (Component) getMod().translate(key).getRaw();
	}
	
	@NonNull
	private Function<String, Optional<Component>> map(@NonNull Function<String, Optional<IComponent>> fct){
		return str -> fct.apply(str).map(IComponent::getRaw).map(Component.class::cast);
	}
	
	@NonNull
	protected Component[] getTooltips(@Nullable String category, @NonNull String fieldName){
