...
public class ToolConfiguration{
	private static final String[] DESC_IGNORE_TOOLS = {
			"When set to true, the mod will be activated",
			"no matter what you have in your hand (or empty hand).",
			"",
	...
	private static final String[] DESC_SPEED_MULTIPLICAND = {
			...
			"",
			"0 will disable this, so the speed will be the default one",
			"of breaking a block.",
			"",
			"If set to 1 each log block will be counted once, so if the",
			"tree is 5 blocks tall it'll require the time of breaking 5 logs.",
			"",
			"If set to 2 each log block will be counted twice, so if the",
			"tree is 5 blocks tall, it'll require the time of breaking 10 logs",
			"",
			"INFO: Only in INSTANTANEOUS mode.",
			"WARNING: If you are on a server, this either has to be set to 0",
			"or every player should have the mod. Else they'll have a weird",
			"effect of breaking the block but the block is still there."
	};
	private static final String[] DESC_PRESERVE = {
			"When set to true, when a tree is broken and the tool is about",
			"to break we will just break enough blocks so that the tool is",
			"left at 1 of durability."
	};
	...
	public void fillConfigScreen(ConfigBuilder builder){
		BooleanListEntry ignoreToolsEntry = builder.entryBuilder()
				...
				.setTooltip(toTooltips(DESC_IGNORE_TOOLS))
		...
		StringListListEntry whitelistedEntry = builder.entryBuilder()
				...
				.setTooltip(toTooltips(DESC_WHITELISTED))
		...
		StringListListEntry blacklistedEntry = builder.entryBuilder()
				...
				.setDefaultValue(Lists.newArrayList())
				.setTooltip(toTooltips(DESC_BLACKLISTED))
		...
		IntegerListEntry damageMultiplicandEntry = builder.entryBuilder()
				...
				.setDefaultValue(1)
				.setMin(0)
				.setTooltip(toTooltips(DESC_DAMAGE_MULTIPLICAND))
		...
		DoubleListEntry speedMultiplicandEntry = builder.entryBuilder()
				...
				.setMin(0)
				.setMax(50)
				.setTooltip(toTooltips(DESC_SPEED_MULTIPLICAND))
		...
		BooleanListEntry preserveEntry = builder.entryBuilder()
				...
				.setDefaultValue(false)
				.setTooltip(toTooltips(DESC_PRESERVE))
