...
public class TreeConfiguration{
	...
	public TreeConfiguration(ForgeConfigSpec.Builder builder){
		breakMode = builder.comment("How to break the tree. Instantaneous will break it in one go. Shift down will make the tree fall down as you cut it, so you still have to break x blocks but don't have to climb the tree for them.").defineEnum("break_mode", BreakMode.INSTANTANEOUS);
		...
		maxSize = builder.comment("The maximum size of a tree. If there's more logs than this value the tree won't be cut. (only in INSTANTANEOUS mode)").defineInRange("logs_max_count", 100, 1, Integer.MAX_VALUE);
		...
		lavesBreakingForceRadius = builder.comment("Radius to force break leaves. If another tree is still holding the leaves they'll still be broken. If the leaves are persistent (placed by player) they'll also be destroyed. The radius is applied from one of the top most log blocks. break_leaves must be activated for this to take effect. (only in INSTANTANEOUS mode)").defineInRange("leaves_breaking_force_radius", 0, 0, 10);
